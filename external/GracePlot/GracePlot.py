...
Slightly updated by Marcus H. Mendenhall, Vanderbilt University, to allow some class overrides, including the underlying grace_np
Further updated November 8, 2008 by MHM to correctly handle line styles & symbol styles in multi-graph environments.  All styles used to go to G0.

__author__ = "John Kitchin" (no longer active)

Maintenance of this project was taken over by Marus Mendenhall in April, 2009.

The GracePlot instance no longer depends on any grace_np process as of April 1, 2009.  Instead, it communicates via subprocess.Popen.
This renders the package incompatible with python < 2.4.

Very small extensions with respect to the original Revision 1.44 have been added by Ralf Hambach, 2013. 
This file is redistributed under GPLv2 (see Licence-file). The original project can be found at 
https://sourceforge.net/projects/graceplot
"""

__version__ = "$Revision: 1.44 (+slight modifications)$"
__author__ = "Marcus Mendenhall <mendenhall@users.sourceforge.net> (maintainer), small modifications by Ralf Hambach"
__date__ = "$Date: 2013/07/30 21:35:22 $"
...
class symbols:
    (none, circle, square, diamond, triangle_up, triangle_left, triangle_down, triangle_right,
        plus, cross, star, character) = range(12)
...
class GracePlot:   
    ...
    def __init__(self,width=None,height=None, auto_redraw=True, redraw_interval=0.1, rows=1, cols=1, scalex=0.9, scaley=0.9):
        """ 
        ...
        width*resolution=width in pixels, same for height.  Resolution is set to 72 by default,
        so width is roughly inches on an old 72 dpi monitor.  By changing the class default resolution,
        ...
        The GracePlot class does all its data transmission through a thread, so normally there should be no 
        ...
        args = args + ('-dpipe', '0') #the -pipe method to get stdin freezes grace until EOF, -dpipe works right
        ...
        transmitter=threading.Thread(target=_sender, args=(self._transmit_queue, self.grace.stdin, redraw_interval, auto_redraw))
        ...
        x0=0.15;  y0=0.15;
        dx=0.8/cols;  dy=0.8/rows;
        for r in range(rows):
          for c in range(cols):
            self.new_graph(view_xmin=x0+c*dx, view_xmax=x0+(c+scalex)*dx, \
                           view_ymin=y0+r*dy, view_ymax=y0+(r+scaley)*dy);
    ...
    def focus(self, graph_index=None, grace_graph=None):
        """direct commands sent to the GracePlot to the appropriate GraceGraph... mostly for backwards compatibility. 
    ...
    def plot(self, *args, **kwargs):
        """shortcut for sending the plot command directly to the appropriate graceGraph object"""
    ...
    def save(self, filename, format='agr', orientation='landscape'):
        ...
        # to be basically the same each time, and each device is subtly different
            ...
            if orientation=='landscape':
                ...
                self.send_commands('PAGE RESIZE 400,600', 'device "PNG" dpi 600',
                                   'hardcopy device "PNG"', 'print to "%s"' % filename, 'print')
            else:
                raise ValueError, 'orientation should be "protrait" or "landscape"'
        elif ext == '.ps': 
            if orientation=='landscape':
                self.send_commands('DEVICE "PostScript" PAGE SIZE 842,595',
                                   'hardcopy device "PostScript"', 'print to "%s"' % filename, 'print')
            elif orientation=='portrait':
                self.send_commands('DEVICE "PostScript" PAGE SIZE 595,842', 
                                   'hardcopy device "PostScript"', 'print to "%s"' % filename, 'print')
            else:
                raise ValueError, 'orientation should be "protrait" or "landscape"'
...
class GraceGraph:
    ...
    """
    def __init__(self, grace, gID,view_xmin=0.15,view_xmax=None,view_ymin=0.15,view_ymax=None):
...
class Line:
    ...
    def __init__(self,type=None,linestyle=None,linewidth=None,color=None,pattern=None,
                 ...
                 filltype=None,fillrule=None,fillcolor=None,fillpattern=None):
    ...
    def output(self,dataset):
        ...
        if self.filltype is not None:
            list.append(dataset+" fill type %d" % self.filltype)
        if self.fillrule is not None:
            list.append(dataset+" fill rule %d" % self.fillrule)
        if self.fillcolor is not None:
            list.append(dataset+" fill color %s" % self.fillcolor)
        if self.fillpattern is not None:
            list.append(dataset+" fill pattern %s" % self.fillpattern)
        return list
...
class Tick:
    ...
    default is ? a number
    spectype is 'ticks' or 'both' of special ticks are defined
    TickSpecs is a list of TickSpec points
    """
    ...
    def output(self,axis):
        ...
        if self.TickLabel is not None:
            list.extend(self.TickLabel.output(axis))
        if self.spectype is not None:
            list.append(axis+' tick spec type %s' % self.spectype)
            list.append(axis+' tick spec %d' % len(self.TickSpecs))
            for num,tick in enumerate(self.TickSpecs):
                list.extend(tick.output(axis,num))
...
class TickSpec:
    def __init__(self,pos,type="major",label=None):
        """
         single special tick
         pos   position on axis
         type  "minor" or "major"
         label 
        """
        self.pos  = pos;
    ...
    def output(self,axis,num):
        " num must by a consecutive number for enumerating the ticks"
        list=[]
        list.append(axis+' tick %s %d, %f'% (self.type, num, self.pos))
        if self.label is not None:
          list.append(axis+' ticklabel %d, "%s"' % (num, self.label))
